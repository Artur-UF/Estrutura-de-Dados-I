...
void imprimeAux(GrafoPLA *grafo, int chOri, int chave){
	if(chOri != chave)	imprimeAux(grafo, chOri, grafo->vertices[chave].pai);
	printf("%d-", chave);
}

void imprimeCaminhoMinimoPesadoGrafoPLA(GrafoPLA *grafo, int chOri, int chDest){
	imprimeAux(grafo, chOri, chDest);
	printf("\n");		
}
...
float caminhoMinimoPesadoComParadaGrafoPLA(GrafoPLA *grafo, int chOri, int chInt, int chDest){
	...
	if(grafo->vertices[chInt].pai == -1) return -1;

	imprimeAux(grafo, chOri, grafo->vertices[chInt].pai);
	distancia = grafo->vertices[chInt].distInicio;

	dijkstraGrafoPLA(grafo, chInt, chDest);
	if(grafo->vertices[chDest].pai == -1) return -1;	

	imprimeAux(grafo, chInt, chDest);
	printf("\n");
